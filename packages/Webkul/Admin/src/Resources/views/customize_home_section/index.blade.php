    ...
    <div class="content">
        ...
        <div class="page-header">
            <div class="page-title">
                <h1>{{ __('admin::app.cms.customize_home_section.config') }}</h1>
            </div>
            ...
            <div class="control-group">
                            ...
                            @endif
                                <div class="category_select_group">
                                    <div class="category_select_header">
                                        <span class="panel_head_name panel_title_{{ $i }}">{{ $value['section_name'] }}</span>
                                    ...
                                    <div class="category_select_body hide">
                                        <div class="custom_control_group">
                                            ...
                                            <div class="right_custom_control">
                                                <input readonly value="" id="category_{{ $i }}" data-product-key="product_search_{{ $i }}"
                                                       data-panel-title="panel_title_{{ $i }}" type="text"
                                                       class="dt2 custom_control custom_select2_input" placeholder="Category">
                                                <input id="category_id_{{ $i }}" name="category_id_{{ $i }}" class="value_fill" value="" type="hidden">
                                            </div>
                                        </div>
                                        <div class="custom_control_group">
                                            <div class="left_custom_control">
                                        ...
                                        <div class="custom_control_group">
                                            ...
                                            <div class="right_custom_control">
                                                <input value="{{ $value['section_name'] }}" type="text" id="display_name_{{ $value['section_name'] }}"
                                                       name="display_name_{{ $i }}" class="custom_control" placeholder="Display Name">
                                            </div>
                                        </div>
                                        <div class="custom_control_group">
                                            <div class="left_custom_control">
                                                <label for="">Visible Condition</label>
                                            </div>
                                            <div class="right_custom_control">
                                                <label for="is_visible"><input name="is_visible_{{ $i }}" {{ $value['is_visible'] == 1 ? 'checked' : '' }} id="is_visible" type="checkbox" /> Is Visible ?</label>
                                            </div>
                                        </div>
                                        <div class="custom_control_group">
                                            <div class="left_custom_control">
                                            ...
                                            <div class="right_custom_control">
                                                <label for="Subcategory_{{ $i }}"><input {{ $value['display_block_type'] == 1 ? 'checked' : '' }} name="is_subcategory_{{ $i }}" id="Subcategory_{{ $i }}" class="is_sub_cat" type="checkbox" /> Subcategory</label>
                                                <label for="Product_{{ $i }}"><input {{ $value['display_block_type'] == 2 ? 'checked' : '' }} name="is_product_{{ $i }}" class="is_product" id="Product_{{ $i }}" type="checkbox" /> Product</label>
                                            </div>
                                        </div>
                                        ...
                                        <div class="custom_control_group">
                                            ...
                                            <div class="dt2 right_custom_control">
                                                <div class="dt2 multi_select_parent_div">
                                                    @foreach($value['data'] as $product_key => $product_value)
                                                        <button class="multi_select_btn">{{ $product_value->display_name }}
                                                            <i class="fa fa-times close_product"></i>
                                                            <input type="hidden" name="product_id_{{ $i }}[]" value="{{ $product_value->display_category_id }}">
                                                        </button>
                                                    @endforeach
                ...
                <div class="dt2 multi_select_div hide" data-top="">
                    ...
                    <ul class="dt2 multi_select_ul">
                        ...
                        @foreach($catalog_object['categories'] as $key => $value)
                            <li data-id="{{ $key }}"
                                data-cat-id="{{ $value['parent_cat_id'] }}"
                                data-field-type="sub-cat"
                                class="dt2 multi_select_li">{{ $value['cat_name'] }}</li>
                        @endforeach
                    </ul>
                </div>
                ...
                <div class=" custom_select2 hide">
                    ...
                    <ul class=" custom_select2_ul">
                        @foreach($catalog_object['categories'] as $key => $value)
                            <li data-id="{{ $key }}"
                                class=" custom_select2_li">{{ $value['cat_name'] }}</li>
                        @endforeach
                        @foreach($parent_category as $key => $value)
                            <li data-id="{{ $key }}"
                                class=" custom_select2_li">{{ $value['parent_name'] }}</li>
                        @endforeach
                    </ul>
                </div>
...
@push('scripts')
    <script>
        $( document ).ready(function() {
            var parent_category = JSON.parse(JSON.stringify({!! json_encode($parent_category) !!}));
            console.log('len',  parent_category);
            ...
            $('form.insert').submit(function(event) {
                ...
                $.ajax({
                    url: '{{ route('admin.customize_home_section.store') }}',
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
...
//                        location.reload();
            ...
            $("body").on("focus", ".multi_select", function (e) {
                ...
                // data-field-type
                var is_product = $(e.target).parent().parent().parent().parent().find(".is_product").is(':checked');
                var is_sub_cat = $(e.target).parent().parent().parent().parent().find(".is_sub_cat").is(':checked');
                var cond = is_product ? "product" : "sub-cat";
//                console.log(is_product, is_sub_cat)

                var div = $("ul."+$(this).attr("data-ul"));
                for (var i = 0; i < div.children().length; i++) {
                    var li_cat = div.children()[i].getAttribute("data-cat-id");
                    var li_cat_an = div.children()[i].getAttribute("data-field-type");
                    console.log($("#"+($(this).attr("data-cat-name"))).val(), li_cat)
                    if ((li_cat == $("#"+($(this).attr("data-cat-name"))).val()) && (li_cat_an == cond)) {
                        div.children()[i].classList.remove('hide');
                    } else {
                        $("ul."+$(this).attr("data-ul")).children()[i].classList.add('hide');
                    }
...
//                    multi_select_div.css("top", (top+30) + "px");
            ...
            $("#add_new_category").on("click", function (e) {
                var count = $("#newly_added_div").children().length >= 0 ? $("#newly_added_div").children().length-1 : 0;
                var div = '<div class="category_select_group">'+
                            ...
                            '<div class="custom_control_group">'+
                                ...
                                '</div>'+
                                ' <div class="right_custom_control">'+
                                    '  <input value="" type="text" id="display_name_'+count+'"'+
                                    ' name="display_name_'+count+'" class="custom_control" placeholder="Display Name">'+
                                ' </div>'+
                            '</div>'+
                            '<div class="custom_control_group">'+
                                '<div class="left_custom_control">'+
                                ...
                                '</div>'+
                            '<div class="right_custom_control">'+
                                '<label for="is_visible_'+count+'"><input name="is_visible_'+count+'"  id="is_visible_'+count+'" type="checkbox" /> Is Visible ?</label>'+
                                    '</div>'+
                            '</div>'+
                            '<div class="custom_control_group">'+
                                ' <div class="left_custom_control">'+
                                ...
                                ' </div>'+
                                '<div class="right_custom_control">'+
                                    '<label for="Subcategory_'+count+'"><input name="is_subcategory_'+count+'" class="is_sub_cat" id="Subcategory_'+count+'" type="checkbox" /> Subcategory</label>'+
                                    '<label for="Product_'+count+'"><input name="is_product_'+count+'" class="is_product" id="Product_'+count+'" type="checkbox" /> Product</label>'+
                                '</div>'+
                            '</div>'+
                            '<div class="custom_control_group">'+
                                '<div class="left_custom_control">'+
